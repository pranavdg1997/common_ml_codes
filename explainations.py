...
def regression_explanations(model,
                            train_df,
                            val_df,
                            regressors,
                            model_variable,
                            method = "MAPLE"):
    if(method == "MAPLE"):
        X_train, X_val = train_df[regressors].values, val_df[regressors].values
        Y_train, Y_val = train_df[model_variable].values, val_df[model_variable].values
        params = model.get_params()                       
        exp_maple = MAPLE(X_train,
                          Y_train,
                          X_val,
                          Y_val,
        ...
        coefs_maple = np.asarray([exp_maple.explain(X_val[i,:])["coefs"][1:] for i in range(X_val.shape[0])])
        exp_values = np.sum(np.multiply(coefs_maple, X_val), axis= 0)
        print(exp_values.shape)
        exps = {}
        y_total = np.sum(Y_val)
        for i in range(len(regressors)):
            reg = regressors[i]
            exps[reg] = round_up((exp_values[i] / y_total)*100)
        exps["unexplained"] = 100 - np.sum(list(exps.values()))
        return(exps)
